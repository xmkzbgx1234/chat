    ...
    if (user.getId() == id && user.getPassword() == password)
    {
        ...
        if(!groups.empty()){
            ...
            for(Group &group : groups){
                json group_js; // 外层：群组JSON，重命名避免混淆
                ...
                for(groupUser &user : users){
                    json user_js; // 内层：成员JSON，重命名
                    user_js["id"] = user.getId();
                    user_js["name"] = user.getName();
                    user_js["role"] = user.getRole(); // 群角色（creator/normal）
                    user_js["state"] = user.getState(); // 用户在线状态（online/offline）
